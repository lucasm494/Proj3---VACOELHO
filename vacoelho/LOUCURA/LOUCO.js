...
function setup(shaders) {
    ...
    let previousPositionX = data.position.x; // Track the object's previous X position
    let previousPositionZ = data.position.z; // Track the object's previous X position
    let previousRotationY = data.rotation.y; // Track the object's previous Y rotation
    ...
    positionFolder.add(data.position, "x", -1.5, 1.5, 0.1).name("X").listen().onChange((newX) => {
        const deltaX = newX - previousPositionX; // Calculate the change in position
        data.position.x = newX; // Update the object's position
        lightsData.objectLight.position.x += deltaX; // Apply the delta to the light's position
        previousPositionX = newX; // Update the previous position
      });
    positionFolder.add(data.position, "y", 0, 2, 0.1).name("Y").domElement.style.pointerEvents = "none";;
    positionFolder.add(data.position, "z", -1.5, 1.5, 0.1).name("Z").listen().onChange((newZ) => {
        const deltaZ = newZ - previousPositionZ; // Calculate the change in position
        data.position.z = newZ; // Update the object's position
        lightsData.objectLight.position.z += deltaZ; // Apply the delta to the light's position
        previousPositionZ = newZ; // Update the previous position
      });;
    ...
    rotationFolder.add(data.rotation, "y", -Math.PI, Math.PI, 0.1).name("Y").onChange((newYRotation) => {
        const deltaYRotation = newYRotation - previousRotationY; // Calculate the change in rotation
        data.rotation.y = newYRotation; // Update the object's Y-axis rotation
    
        // Update the light's position based on rotation
        const radius = Math.sqrt(
          Math.pow(lightsData.objectLight.position.x, 2) +
            Math.pow(lightsData.objectLight.position.z, 2)
        ); // Calculate the distance from the origin to the light
        const angle = Math.atan2(
          lightsData.objectLight.position.z,
          lightsData.objectLight.position.x
        ); // Calculate the current angle of the light
    
        // Add the rotation change
        const newAngle = angle + deltaYRotation;
    
        // Update light's position
        lightsData.objectLight.position.x = radius * Math.cos(newAngle);
        lightsData.objectLight.position.z = radius * Math.sin(newAngle);
    
        previousRotationY = newYRotation; // Update the previous rotation
      });;
    ...
    function objectLight() {
        ...
        STACK.multScale([0.2, 0.2, 0.2]); // Apply scale
        uploadModelView();
        // Draw the selected object
